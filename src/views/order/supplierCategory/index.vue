  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['order:supplierCategory:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    >
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" min-width="120" >
        <template #default="scope">
          <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['order:supplierCategory:add']">新增</el-button>
          <el-button link type="success" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['order:supplierCategory:edit']">修改</el-button>
          <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['order:supplierCategory:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="supplierCategoryRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="父级ID" prop="parentId">
          <el-tree-select
            v-model="form.parentId"
            :data="supplierCategoryOptions"
...
<script setup name="SupplierCategory">
import { listSupplierCategory, getSupplierCategory, delSupplierCategory, addSupplierCategory, updateSupplierCategory, changeStatus } from "@/api/order/supplierCategory";
...
function getList() {
  ...
  listSupplierCategory(queryParams.value).then(response => {
    supplierCategoryList.value = proxy.handleTree(response.data, "categoryId", "parentId");
    loading.value = false;
  });
}
...
function getTreeselect() {
  listSupplierCategory().then(response => {
    ...
    supplierCategoryOptions.value.push(data);
  });
}
...
function reset() {
  ...
  proxy.resetForm("supplierCategoryRef");
}
...
async function handleUpdate(row) {
  ...
  getSupplierCategory(row.categoryId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["supplierCategoryRef"].validate(valid => {
    if (valid) {
      if (form.value.categoryId != null) {
        updateSupplierCategory(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addSupplierCategory(form.value).then(response => {
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除供应商分类编号为"' + row.categoryId + '"的数据项？').then(function() {
    return delSupplierCategory(row.categoryId);
  }).then(() => {
    getList();
