  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="商品分类:" prop="categoryId">
        <el-tree-select v-model="queryParams.categoryId" :data="categoryList"
    ...
    <el-table v-loading="loading" :data="productList" @selection-change="handleSelectionChange" fit>
      ...
      <el-table-column label="商品主图" align="center" prop="productImage" width="80">
      ...
      <el-table-column label="sku规格" header-align="center" min-width="200" show-overflow-tooltip>
        <template #default="scope">
        ...
        <el-card>
          ...
          <el-descriptions direction="vertical" :column="6" :size="size" border style="margin-top:20px ;">
            <el-descriptions-item label="商品分类" :span="2" align="center">
              {{ getCategoryName(productDetail.categoryId) ? productDetail.categoryId : '--' }}
            </el-descriptions-item>
            <el-descriptions-item label="品牌" align="center">
              {{ getBrandName(productDetail.brandId) ? productDetail.brandId : '--' }}
            </el-descriptions-item>
      ...
      <div>
        <el-card>
          ...
          <el-descriptions direction="vertical" :column="5" :size="size" border style="margin-top:20px ;">
            ...
            <el-descriptions-item label="宽(cm)" align="center"> {{ productDetail.width ? productDetail.width : '--' }}
              cm </el-descriptions-item>
            <el-descriptions-item label="高(cm)" align="center"> {{ productDetail.height ? productDetail.height : '--' }}
              cm </el-descriptions-item>
            <el-descriptions-item label="体积(m³)" align="center"> {{ formatTree(productDetail.volume) }} m³
            </el-descriptions-item>
            <el-descriptions-item label="重量(kg)" align="center"> {{ formatTree(productDetail.weight) }} kg
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
      </div>
            ...
            <el-table-column label="规格值" align="center" show-overflow-tooltip>
              <template #default="scope">
                <div v-for="(item,index) in getSkuValue(scope.row.skuValue) " :key="index">
                  <strong v-if="item[0]"> {{ item[0] }}: </strong>
                  <span> {{ item[1]}}</span>
                </div>
              </template>
            </el-table-column>
...
const getCategoryName = (categoryId) =>{
  const category = categoryNameList.value.find(item => item.categoryId === categoryId)
