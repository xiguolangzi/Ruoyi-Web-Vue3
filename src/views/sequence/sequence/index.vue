  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="排序规则" prop="sequenceRule">
        <el-select v-model="queryParams.sequenceRule" placeholder="请选择排序规则" clearable>
          <el-option
            v-for="dict in order_sequence_rules"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['sequence:sequence:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['sequence:sequence:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['sequence:sequence:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['sequence:sequence:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="sequenceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" min-width="110">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['sequence:sequence:edit']">修改</el-button>
          <el-button link type="danger" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['sequence:sequence:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="sequenceRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="订单前缀" prop="sequencePrefix">
          <el-input v-model="form.sequencePrefix" placeholder="请输入订单前缀" />
        </el-form-item>
        ...
        <el-table :data="sequenceDetailList" :row-class-name="rowSequenceDetailIndex" @selection-change="handleSequenceDetailSelectionChange" ref="sequenceDetail">
          ...
          <el-table-column label="会计月份" prop="periodMonth" >
            <template #default="scope">
              <el-select v-model="scope.row.periodMonth" placeholder="请选择会计月份" style="width: 100%;">
                <el-option
...
const { project_general_status, finance_period_year, order_sequence_type, order_sequence_date_type, finance_period_month, order_sequence_rules } = proxy.useDict('project_general_status', 'finance_period_year', 'order_sequence_type', 'order_sequence_date_type', 'finance_period_month', 'order_sequence_rules');
...
const handleRulesChanged = () => {
  if (form.value.sequenceRule === '1') {
    form.value.dateType = 'yyyy';
  } else if (form.value.sequenceRule === '2') {
    form.value.dateType = 'yyyyMM';
  } else if (form.value.sequenceRule === '3') {
    form.value.dateType = 'yyyyMMdd';
  }
}
